...
$(function () {
    ...
    $.post(https + '/api/live-room/vn/?code=' + Math.random(), function (data) {
        ...
        if (roomList[v] && t && v && (roomList[v].title === decodeURI(t))) {
            ...
            $('#video-title').html(defData.title + (playStatus === 1 ? '<span style="color: #00a65a;margin: 0 20px;font-size: 20px;">phát trực tiếp</span>' : '<span style="color: darkred;margin: 0 20px;font-size: 20px;">tạm thời rời đi</span>'));
            v = defData.uuid;
            t = defData.title;
